    ...
    $.ajax({
        ...
        success:function(data){
            ...
            for(var count = 0; count<allos_data.length; count++){
                ...
                allos += '<h2 class="title">' + cell_data[0] + '</title>';
                allos += '<p class="copy">' + cell_data[1] + '</p>';
                allos += '<style>.allo:nth-child('+(count+1)+'):before { background-image: url(../../../data/allos/' + cell_data[2] + '.jpg);}</style>'
                allos += '</div>';
